...
from pyconfigue.providers import StaticFileProvider
...
class TestStaticFileProvider:
    ...
    def test_get_single_configue():
        provider = StaticFileProvider( TEST_FILE1)
        provider.set_model_validator(ModelValidatorTest)
        assert provider.model_validator is not None
        assert provider.get(TEST_KEY) == TEST_VALUE

    @staticmethod
    def test_get_multiple_configue():
        provider = StaticFileProvider( [TEST_FILE1, TEST_FILE2])
        provider.set_model_validator(ModelValidatorTest)
